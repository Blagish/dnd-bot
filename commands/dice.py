...
class Dice(commands.Cog, name='Кубы кубы'):
    ...
    async def roll(self, ctx, *, string):
        ...
        Поддерживаются математические функции минимума, максимума и суммы - **min**, **max**, **sum**. Пример: min(d69, d420).
        Можно указывать несколько команд через запятую! "/roll d20+10, 5d8+2, 2d6+12"

        Вы можете указать после выражения небольшой комментарий - например, тип урона или тему броска. Даника автоматически сгруппирует кубы с одинаковым комментарием, например: 2д6 режущ + д8 огонь + д6 кислота + д10 огонь выдаст что-то вроде "(7 режущ, 14 огонь, 3 кислота). Это можно использовать, если у монстров есть сопротивление или уязвимость к определенным видам урона, ну или просто для удобства, как тут: д20+6 атака, д20+3 инициатива, д20+8 скрытноть и т.д. **ВНИМАНИЕ** комментарий должен быть без пробелов, единым словом, и не содержать в себе чисел и специальных знаков.
