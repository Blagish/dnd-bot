...
def help_list(*args):
    s = '''Чтобы кидать дайсы, используйте символ d(d20, 4d6+1). Чтобы кидать с преимуществом, супер-преимуществом ( 
    оно же эльфийское) или с помехой, используйте обозначения ad, ed или dd соответственно (ad20, ed20, 
    dd20). Доступны: основные математические операции, кубы, преимущество, помеха, эльфийский, четверной. Сравнение 
    выражений знаками больше и т.д. Проверка выражения на истинность. Например: d20+10>20?d6+3:0, то есть выполнится 
    первое если истина и второе если ложь. Использование скобок. Использование нескольких команд разом: 5x(d20+10), 
    скобки пока обязательны, можно использовать только число как коэффициент. Использование математических функций 
    sum, min, max. Разрушается делать несколько бросков за раз, перечисляя команды кубов через запятую. Разрешается 
    запускать несколько команд в одно сообщении, разделяя их точкой с запятой. '''
    return s
