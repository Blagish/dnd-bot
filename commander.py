...
@handler('help_beta', ['хуйхелп', 'hidden'])
def help_beta(*args):
    s = '''Бета-тест нового парсера производится при помощи команды /хуй {выражение}. Доступны: основные 
    математические операции, кубы, преимущество, помеха, эльфийский, четверной. Сравнение выражений знаками больше и 
    т.д. Проверка выражения на истинность. Например: d20+10>20?d6+3:0, то есть выполнится первое если истина и второе 
    если ложь. Использование скобок. Использование нескольких команд разом: 5x(d20+10), скобки пока обязательны, 
    можно использовать только число как коэффициент. Использование математических функций sum, min, max. 
    В планах: поддержка комментариев, модификаторов типа ea, gwf, rt, чета еще что было забыто но на то это и тест.'''
    return s
