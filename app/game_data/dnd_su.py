...
def parse_content(element, ignore_br=True):
    ...
    if element.name in tags_with_new_strings:
        style2 = "\n"
        if element.name == "li" and element.attrs.get("class") == [
            "size-type-alignment"
        ]:
            style1 = "> *"
            style2 = "*\n"
        elif element.name == "li" and element.attrs.get("class") != [
            "subsection",
            "desc",
        ]:
            style1 = "> "
...
def get_spell(name):
    ...
    embed_card = Embed(title=title, url=target_url, description=content, colour=COLOUR)
