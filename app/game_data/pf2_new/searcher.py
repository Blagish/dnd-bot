...
class SpellIndexer:
            ...
            try:
                ...
                if spell_data and "name" in spell_data:
                    ...
                    if spell_name in self.spell_index:
                        print(
                            f"Предупреждение: заклинание '{spell_name}' уже существует! "
    ...
    def search_spells_advanced(
        self, query: str, include_description: bool = False, max_results: int = 50
    ) -> list[dict]:
        """
        ...
        for spell_name, spell_path in self.spell_index.items():
            ...
            spell_data = self._load_spell_data(
                Path(self.packs_directory.parent) / spell_path
            )
            if spell_data:
                description = (
                    spell_data.get("system", {}).get("description", {}).get("value", "")
                )
...
def search_spells_advanced(
    query: str, include_description: bool = False, max_results: int = 50
) -> list[dict]:
    """
    ...
    return _spell_indexer.search_spells_advanced(
        query, include_description, max_results
    )
